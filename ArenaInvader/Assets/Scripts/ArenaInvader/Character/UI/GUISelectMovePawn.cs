...
public class GUISelectMovePawn : GUINetworkPopUp<int> {
    ...
    private GamePawn _selectedPawn;
    private int _amount;

    public void SetPlayer(IPlayableCharacter target, int amount, int addedAmount)
    {
        _target = target;
    ...
    private GamePawn GetPredictPawn()
    {
    ...
    private void Update()
    {
        ...
        GamePawn newPawn = GetPredictPawn();
    ...
    public void FocusPawn(int value)
    {
        ...
        _selectedPawn = _target.PawnOwner.Pawns[value];
    ...
    public void MovePawn(int idx) {
        _pawnPredict.SetActive(false);
        _target.PawnOwner.MovePawn(idx, _amount);
        TryClose();

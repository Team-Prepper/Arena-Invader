...
public class GamePawn : Pawn {
    ...
    {
        if (_piggyBacking != null) PawnToGamePawn(_piggyBacking).AddAttack(attackAmount);
        _attackValue += attackAmount * _attackCoefficient;
    }

    public void AddDefence(int defenceAmount)
    {
        if (_piggyBacking != null) PawnToGamePawn(_piggyBacking).AddDefence(defenceAmount);
        _defenceValue += defenceAmount * _defenseCoefficient;
    }

    public void AddHealth(int healAmount)
    {
        if (_piggyBacking != null) PawnToGamePawn(_piggyBacking).AddHealth(healAmount);
